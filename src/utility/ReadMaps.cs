...
namespace Landis.Extension.Succession.NECN
{
    ...
    public static class ReadMaps
    {
        ...
        public static void ReadManagementAreaMap(string managementPath)
        {
            IInputRaster<DoublePixel> map = MakeDoubleMap(managementPath);
            ...
            using (map)
            {
                ...
                foreach (Site site in PlugIn.ModelCore.Landscape.AllSites)
                {
                    ...
                    if (site.IsActive)
                    {
                        if (mapValue < 0.0 || mapValue > 75000.0)
                            throw new InputValueException(mapValue.ToString(),
                                                          "managementMap value {0} is not between {1:0.0} and {2:0.0}. Site_Row={3:0}, Site_Column={4:0}",
                                                          mapValue, 0.0, 75000.0, site.Location.Row, site.Location.Column);
                        SiteVars.ManagementCode[site] = mapValue;
                    }
                }
            }
        }
